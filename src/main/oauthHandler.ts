...
function describeNetworkError(err: unknown): string {
  ...
  else if (anyErr.cause != null) parts.push(`cause: ${String(anyErr.cause)}`);
  if (anyErr.code) parts.push(`code: ${anyErr.code}`);
  return parts.join(' — ');
}

async function oauthNetFetch(url: string, init?: RequestInit, retries = 1): Promise<Response> {
  let lastErr: unknown;
...
async function oauthFetchUntilOk(
  label: string,
  url: string,
  init: RequestInit,
  maxAttempts = 6
  ...
  for (let i = 1; i <= maxAttempts; i++) {
    ...
    console.warn(
      `[OAuth] ${label} HTTP ${res.status}, attempt ${i}/${maxAttempts}, retry in ${Math.round(delay)}ms`
    );
...
async function oauthFetchTransientRetry(
  label: string,
  url: string,
  init: RequestInit,
  maxAttempts = 5
...
export class OAuthHandler {
  private static oauthWindow: BrowserWindow | null = null;
  ...
  private static readonly PARTITION = 'persist:msa-oauth';
  ...
  static async clearSession(): Promise<void> {
    try {
      const part = session.fromPartition(this.PARTITION);
  ...
  static async authenticate(forceAccountPicker = true): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (forceAccountPicker) params.set('prompt', 'select_account');
  ...
  private static handleAuthCode(code: string) {
    console.log('========================================');
  ...
  private static async exchangeCodeForToken(code: string): Promise<any> {
    try {
      ...
      const tokenResponse = await oauthFetchUntilOk(
      ...
      const xboxResponse = await oauthFetchUntilOk(
      ...
      const xstsResponse = await oauthFetchUntilOk(
        'XSTS token retrieval',
      ...
      const mcLoginResponse = await oauthFetchUntilOk(
        'Minecraft login',
      ...
      try {
        const entitlementsResponse = await oauthFetchUntilOk(
        ...
        console.log(
          'Entitlements response items:',
          JSON.stringify(entitlementsData.items?.map((i: any) => i.itemname))
        );
        ownsMinecraft =
          entitlementsData.items &&
          entitlementsData.items.some(
            (item: any) =>
              item.itemname === 'product_minecraft' ||
      ...
      if (ownsMinecraft) {
        try {
          const profileResponse = await oauthFetchUntilOk(
            'Minecraft profile',
  ...
  static setupIpcHandlers() {
    ipcMain.handle('oauth:authenticate', async (_e, opts?: { forceAccountPicker?: boolean }) => {
      return await this.authenticate(opts?.forceAccountPicker !== false);
    });
    ipcMain.handle('oauth:signOut', async () => {
