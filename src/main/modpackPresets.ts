...
interface PresetItem {
  ...
  /** If true, skip this item when the resolve/download fails (don't abort pack install). */
  optional?: boolean;
}
 ...
 */
const FABRIC_RECOMMENDED: PresetItem[] = [
...
async function resolveLatest(
  ...
  const versions = await api<any[]>(`/project/${encodeURIComponent(slug)}/version?${qs.toString()}`);
  // Prefer release over beta/alpha; list is already newest-first.
  return (
    versions.find((v) => v.version_type === 'release') || versions[0] || null
  );
}

export function setupPresetHandlers(getInstance: (id: string) => InstanceLike | null): void {
  ipcMain.handle(
    'presets:installRecommended',
    async (_e, instanceId: string) => {
      const instance = getInstance(instanceId);
      if (!instance) throw new Error('Instance not found');
      if (instance.modLoader !== 'fabric') {
        throw new Error(
          `Recommended pack currently targets Fabric only — this instance uses "${instance.modLoader || 'vanilla'}".`
        );
      }
      ...
      const shadersDir = join(gameDir, 'shaderpacks');
      const packsDir = join(gameDir, 'resourcepacks');
      ...
      let preferredShaderFile: string | null = null;
      ...
      for (const item of FABRIC_RECOMMENDED) {
        try {
          ...
          if (!version) {
            ...
            if (item.optional) skipped.push({ slug: item.slug, reason: msg });
          ...
          const dir =
            item.kind === 'mod' ? modsDir : item.kind === 'shader' ? shadersDir : packsDir;
          const target = join(dir, file.filename);

          try {
            await stat(target);
            installed.push(file.filename);
            if (item.kind === 'shader' && !preferredShaderFile) preferredShaderFile = file.filename;
            continue;
          } catch {
            /* not present */
          }
      ...
      if (preferredShaderFile) {
        try {
          const irisConfigPath = join(gameDir, 'config', 'iris.properties');
